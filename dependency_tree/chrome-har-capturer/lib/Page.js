...
Page.prototype.isDone = function () {
    ...
    //console.error('this.domLoadedTime:' + this.domLoadedTime);
    //console.error('this.endTime:' + this.endTime);
    //console.error('this.originalRequestId:' + this.originalRequestId);
    //console.error('this.originalRequestStatus:' + this.originalRequestStatus);
...
Page.prototype.processMessage = function (message) {
    ...
    case 'Network.requestServedFromCache':
        if (id in this.entries) {
            this.entries[id].fromCache = true;
            break;
        }
        return;
    default:
...
Page.prototype.getHAR = function () {
    ...
    for (var requestId in this.entries) {
        ...
        // skip cached loads (This is different from a push)
        if (entry.fromCache) continue;

        // skip entries with no timing information (it's optional)
        var timing = entry.responseEvent.response.timing;
        if (!timing) continue;

        if (requestId == this.originalRequestId) {
            har['info']['startedDateTime'] = new Date(timing.requestTime * 1000).toISOString();
        }
        ...
        har.entries.push({
            ...
            'request': {
                ...
                'initiatorType': entry.requestEvent.initiator.type,
                'initiatorLineNumber': entry.requestEvent.initiator.lineNumber,
                'initiatorUrl': entry.requestEvent.initiator.url,
                'initiatorStackTrace': entry.requestEvent.initiator.stackTrace,
            },
            'response': {
