...
function save_data(main_url_createdate, id, main_url, object_url, x1, y1, x2, y2, is_css_bg, visible, display, repeat_type, dom_object_type, link_url) {
...
function findLink(obj) {
  var link_url = "null";
  //newText.value = newText.value + "\n Starting upward <a> link search"; 
  do {
    //newText.value = newText.value + "\n name: "+ obj.nodeName + "  obj_recurse: " + obj.src ; 
    ...
    for (var i=0, max=all.length; i < max; i++) {
      ...
      if (obj.nodeName == "IFRAME") {
        try {
          ...
          save_data(main_url_createdate, id, main_url, object_url, x1, y1, x2, y2, 0, visible, display, "null", dom_object_type, link_url);
  ...
  for (var type_num=0; type_num < types.length; type_num++) {
    ...
    while(A.length) {
      ...
      if(object_url) {
        ...
        save_data(main_url_createdate, id, main_url, object_url, x1, y1, x2, y2, 1, visible, display, repeat_type, dom_object_type, link_url);
      }
    }
  }
}
