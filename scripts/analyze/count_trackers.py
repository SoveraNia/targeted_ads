...
def checkArguments():
  if (len(sys.argv) < 2):
    print ("Usage:")
    print ("python count_trackers.py LOG_FILES");
    print ""
    print "Count the number of distinct trackers from the PhantomJS log file";
    sys.exit(0);
...
def main():
    ...
    for j in range(len(lines)):
      ...
      if len(lines[j]) >= 2 and lines[j][0] == '<TRACKER>':
        ...
        if domain in trackers:
          ...
          trackers[domain] = 1;
          num_distinct_trackers += 1;
      elif len(lines[j]) >= 3 and lines[j][1] == 'Tracker Detected:':
        domain = url2Domain(lines[j][2]);
        if domain in trackers:
          trackers[domain] += 1;
        else: 
          trackers[domain] = 1;
  ...
  for key in sorted(trackers.iterkeys()):
    print key,'\t',trackers[key];
